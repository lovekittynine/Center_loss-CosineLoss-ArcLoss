...
# train mnist with cosine loss

import tensorflow as tf
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
from Dataset import make_dataset
from Cosine_loss import get_cosine_loss
import os
import time
import numpy as np
...
def build_model(xs,num_classes=10,print_info=True):
  ...
  with slim.arg_scope([slim.conv2d],
    ...
    # l2 normalize for feature and weights
    logits = fc_layer(net,num_classes=num_classes)
    if print_info:
...
def fc_layer(inputs,num_classes=10):
  # l2_normalize for feature
  normalized_features = tf.nn.l2_normalize(inputs,axis=-1)
  # get feature len
  feature_len = normalized_features.get_shape()[1]
  # last fc layer weights
  init = slim.variance_scaling_initializer()
  weights = tf.get_variable('fc3',initializer=init,shape=[feature_len,num_classes])
  # l2_normalize for weights
  normalized_weights = tf.nn.l2_normalize(weights,axis=0)
  # get logits(cos(theta)) without biases
...
def display_fig(features,targets,epoch):
  if not os.path.exists('./cosine_loss_images'):
    os.makedirs('./cosine_loss_images')
  ...
  fig.savefig('./cosine_loss_images/%d.png'%epoch)
  plt.close(fig)
  
def train():
  
  # make train dataset
  train_dataset = make_dataset(dataType='train',epoch=20,batchsize=256)
  ...
  with tf.name_scope('cosine_loss'):
    loss = get_cosine_loss(logits,ys,s=64,m=0.35)
